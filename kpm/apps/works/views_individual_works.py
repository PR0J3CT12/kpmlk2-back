...
from kpm.apps.grades.models import *
from kpm.apps.groups.models import *
import json
...
@swagger_auto_schema(method='GET', operation_summary="Получение списка самостоятельных работ(пользователь).",
                     responses=get_my_individual_works_responses,
...
def get_my_individual_works(request):
    try:
        ...
        individuals_list = []
        for individual in works_list:
            result = {
                'id': individual,
                'name': works_list[individual]['name'],
                'fields': works_list[individual]['fields'],
                'status': works_list[individual]['status'],
                'course': works_list[individual]['course'],
                'is_done': works_list[individual]['is_done'],
                'is_checked': works_list[individual]['is_checked']
            }
            if works_list[individual]['is_checked']:
                result['score'] = works_list[individual]['score']
                result['max_score'] = works_list[individual]['max_score']
                result['blue'] = works_list[individual]['blue']
                result['green'] = works_list[individual]['green']
            individuals_list.append(result)
        return HttpResponse(json.dumps({'homeworks': individuals_list}, ensure_ascii=False), status=200)
            ...
            return HttpResponse(
                json.dumps(
                    {'state': 'error', 'message': f'Неверно указан курс работы.', 'details': {},
                     'instance': request.path},
                    ensure_ascii=False), status=400)
        query = Q(school_class=int(class_)) & Q(type__in=[2, 10, 11])
        if class_ == '4':
            type_ = '2'
        else:
            if type_ == '2':
...
@swagger_auto_schema(method='PATCH', operation_summary="Проверка индивидуальной работы(админка).",
                     request_body=check_user_individual_work_request_body,
                     responses=check_user_individual_work_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsAdmin']}")
@api_view(["PATCH"])
@permission_classes([IsAdmin, IsEnabled])
def check_user_individual_work(request):
    try:
        if request.body:
...
@swagger_auto_schema(method='POST', operation_summary="Ответ на индивидуальную работу.",
...
@swagger_auto_schema(method='GET', operation_summary="Получение индивидуальной работы(пользователь).",
                     manual_parameters=[id_param],
                     responses=get_user_individual_work_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsAuthenticated']}")
@api_view(["GET"])
@permission_classes([IsAuthenticated, IsEnabled])
def get_user_individual_work(request):
    try:
...
@swagger_auto_schema(method='PATCH', operation_summary="Открыть доступ к самостоятельной работе ученику.",
                     manual_parameters=[id_param, student_param],
                     responses=add_to_individual_work_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsTierOne']}")
@api_view(["PATCH"])
@permission_classes([IsTierOne, IsEnabled])
def add_to_individual_work(request):
    try:
        id_ = get_variable("id", request)
        if (id_ is None) or (id_ == ''):
            return HttpResponse(
...
@swagger_auto_schema(method='PATCH', operation_summary="Закрыть доступ к самостоятельной работе ученику.",
                     manual_parameters=[id_param, student_param],
                     responses=delete_from_individual_work_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsTierOne']}")
@api_view(["PATCH"])
@permission_classes([IsTierOne, IsEnabled])
def delete_from_individual_work(request):
    try:
        id_ = get_variable("id", request)
        if (id_ is None) or (id_ == ''):
            return HttpResponse(
...
@swagger_auto_schema(method='POST', operation_summary="Установить дату проведения самостоятельной работы для группы.",
                     request_body=set_individual_work_date_request_body,
                     responses=set_individual_work_date_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsTierOne']}")
@api_view(["POST"])
@permission_classes([IsTierOne, IsEnabled])
def set_individual_work_date(request):
    try:
        if request.body:
            request_body = json.loads(request.body)
        else:
        ...
        if date > datetime.today() + timedelta(days=365):
            return HttpResponse(
                json.dumps({'state': 'error', 'message': 'Неверная дата. Можно выбрать дату не более чем на год вперед.', 'details': {},
                            'instance': request.path},
                           ensure_ascii=False), status=400)
        group = Group.objects.get(id=group_id)
        work = Work.objects.get(id=work_id, type__in=[2, 10, 11])
...
@swagger_auto_schema(method='DELETE', operation_summary="Удалить дату проведения самостоятельной работы для группы.",
                     manual_parameters=[work_param, group_param],
                     responses=delete_individual_work_date_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsTierOne']}")
@api_view(["DELETE"])
@permission_classes([IsTierOne, IsEnabled])
def delete_individual_work_dates(request):
    try:
...
@swagger_auto_schema(method='GET', operation_summary="Получить даты проведения самостоятельных работ для группы, либо для самостоятельной работы получить даты проведения в группах.",
                     manual_parameters=[work_param, group_param],
                     responses=get_individual_works_dates_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsAdmin']}")
@api_view(["GET"])
@permission_classes([IsAdmin, IsEnabled])
def get_individual_works_dates(request):
    try:
        group_filter = False
        work_filter = False
        group_id = get_variable("group", request)
        ...
        if group_filter:
            group = Group.objects.get(id=group_id)
            rows = GroupWorkDate.objects.filter(group=group, type__in=[2, 10, 11]).values('work_id', 'date')
            for row in rows:
                result.append({
                    'work_id': row['work_id'],
                    'group_id': group_id,
...
@swagger_auto_schema(method='GET', operation_summary="Получение ответов на самостоятельную работу(для таблицы).",
                     manual_parameters=[id_param],
                     responses=get_all_answers_individuals_responses,
                     operation_description=f"Уровни доступа: {permissions_operation_description['IsAdmin']}")
@api_view(["GET"])
@permission_classes([IsAdmin, IsEnabled])
def get_all_answers_individuals(request):
    try:
        id_ = get_variable("id", request)
        if (id_ is None) or (id_ == ''):
            return HttpResponse(
